...
class ConceptNet:
    ...
    def find_adjacent(self, concept):
        if concept not in self.cpt_dict.keys():
            return []
        return list(set(list(self.cpt_dict[concept].keys())))
    ...
    def expand_list(self, concept_list):
        res = []
        index = []
        for cpt in concept_list:
            res.extend(self.find_adjacent(cpt))
    ...
    def expand_list_by_path(self, concept, k):
        time_dict = {}
        for cpt in concept:
            adjacent = self.find_adjacent(cpt)
            for tmp in adjacent:
